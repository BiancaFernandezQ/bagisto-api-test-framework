...
def test_obtener_todos_grupos_si_pagination_es_0(get_token, create_5_groups):
    #url = f"{Endpoint.BASE_GROUP.value}?pagination=0"
    url = Endpoint.BASE_GROUP.value
    headers = {
        "Authorization": f"Bearer {get_token}"
    }
...
@pytest.mark.humo
def test_solicitar_limit_minimo_return_1_grupo(get_token, create_15_customers):
    url = Endpoint.BASE_GROUP.value
    headers = {
        "Authorization": f"Bearer {get_token}"
    }
...
@pytest.mark.positivas
@pytest.mark.humo
def test_solicitar_limit_maximo_return_todos_grupos(get_token, create_15_customers):
    url = Endpoint.BASE_GROUP.value
    headers = {
        "Authorization": f"Bearer {get_token}"
    }
    ...
    json_response = response.json()
    tam_grupos = len(json_response.get("data", []))
    #tam_grupos debe ser igual al total de meta
